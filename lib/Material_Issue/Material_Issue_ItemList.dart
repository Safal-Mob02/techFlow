...
import 'Material_Issue_ListScreen.dart';
...
class ItemsPage extends StatefulWidget {
  String Urn;
  var CategoryCode, categoryName, docNo;
  ItemsPage(
      {Key? key,
      required this.Urn,
      this.CategoryCode,
    ...
    return WillPopScope(
      onWillPop: () async {
        return await Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => const Material_Issue_ListScreen()));
      },
      child: Scaffold(
        backgroundColor: kMainColor,
        resizeToAvoidBottomInset: true,
                                                          ...
                                                          return InkWell(
                                                            onTap: () {
                                                              setState(() {
                                                                ...
                                                                log("CategoryCode ID $CategoryCode");
                                                              });
                                                            },
                                                            child: ListTile(
                                                                title: Text(
                    ...
                    InkWell(
                      ...
                      child: Padding(
                        ...
                        child: Container(
                          ...
                          child: Icon(Icons.add,color: kBorderColorTextField,),
                        ),
                      ),
                    )
                  ],
                                                                  ...
                                                                  padding: EdgeInsets
                                                                      .only(
                                                                          left:
                                                                              5),
                                                                  child: SizedBox(
                                                                    height: 20.0,  // Set a fixed height for the Marquee
                                                                    width: MediaQuery.of(context).size.width/4,
                                                                    child: Marquee(
                                                                      text: DoPandingListData.data[index].productionItem != ""?DoPandingListData.data[index].productionItem:"--",
                                                                      style: TextStyle(
...
// If no insufficient stock found, call SaveData
  ...
  Future<SelectCategoryIssueResponse> GetLocation(SearchText) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (LocationResponseData.settings.success == "1") {
        setState(() {
          if (LocationResponseData.data.isNotEmpty) {
            var defaultItem = LocationResponseData.data.firstWhere(
                    (item) => item.selectValue == "Issue");

            if (defaultItem != null) {
              CategoryController.text = defaultItem.selectValue;  // Default "Issue"
              CategoryCode = defaultItem.selectValueCode;
              log("CategoryController.text>>>${CategoryController.text}");
              log("CategoryCode>>>${CategoryCode}");
              GetURN(CategoryCode);  // Fetch URN for default selection
            }
          }
  ...
  Future<void> DeleteItem(srNo) async {
    ...
    log("Api Name: ${clientUrl}/TechFlow/Material_Issue_Delete_Item $BODYDATA");
    final response = await http.post(
      Uri.parse("${clientUrl}/TechFlow/Material_Issue_Delete_Item"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}/TechFlow/Material_Issue_Delete_Item $BODYDATA");
    log("Response Status Code: ${response.statusCode}");
  ...
  Future<void> fetchdata(SearchText) async {
    ...
    final response = await http.post(
      Uri.parse("${clientUrl}TechFlow/MaterialIssueItemListing"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}TechFlow/MaterialIssueItemListing $BODYDATA");
    log("Response Status Code: ${response.statusCode}");
    log("Response Body: ${response.body}");
    if (response.statusCode == 200) {
      var jsonData = json.decode(response.body);
      ...
      setState(() {
        isLoading = false;
        DoPandingListData = IssueItemListingResponse.fromJson(map);
