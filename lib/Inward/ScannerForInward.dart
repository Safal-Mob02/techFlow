...
import 'Inward_TabPage.dart';


class ScannerForInward extends StatefulWidget {
  var menuName;
  ScannerForInward({Key? key,this.menuName}) : super(key: key);

  @override
  State<ScannerForInward> createState() => _ScannerForInwardState();

}

class _ScannerForInwardState extends State<ScannerForInward> {
  Barcode? result;
  late Tools tools;
  ...
  Future<void> GetURN_FORP(BarcodeRes) async {
    ...
    final response = await http.post(
      Uri.parse("${clientUrl}purchase/PurchaseToInward"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}purchase/PurchaseToInward $BODYDATA");
    ...
    if (response.statusCode == 200) {
      ...
      if (DoPandingListData.settings.success == "1") {
        ...
        setState(() {
          ...
          controller!.stopCamera();
          if(Urn.isNotEmpty){
            Navigator.push(context, MaterialPageRoute(builder: (context) => Inward_TabPage(Urn: Urn,)));
          }
        });
      } else {
        setState(() {
