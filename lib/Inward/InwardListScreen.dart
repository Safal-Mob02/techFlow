...
class _InwardListScreenState extends State<InwardListScreen> {
                                ...
                                child: ListView.builder(
                                    ...
                                    itemBuilder:
                                      ...
                                      return Dismissible(
                                        ...
                                        background: slideLeftBackground(DoPandingListData
                                            .data[
                                        index]
                                            .status),
                                        secondaryBackground:
          ...
          onPressed: () {
            setState(() {
              Navigator.push(context,
                  MaterialPageRoute(builder: (context) => ScannerForInward()));
              //  onNextPageChangeTapped();
            });
          },
          child: Container(
  ...
  Future<void> DeleteItem(Urn) async {
    ...
    log("Api Name: ${clientUrl}/purchase/PurchaseInwardDeletelist $BODYDATA");
    final response = await http.post(
      Uri.parse("${clientUrl}/purchase/PurchaseInwardDeletelist"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}/purchase/PurchaseInwardDeletelist $BODYDATA");
  ...
  Future<void> fetchdata(SearchText) async {
    ...
    final response = await http.post(
      Uri.parse("${clientUrl}/purchase/InwardMasterList"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}/purchase/InwardMasterList $BODYDATA");
    log("Response Status Code: ${response.statusCode}");
    log("Response Body: ${response.body}");
    if (response.statusCode == 200) {
      var jsonData = json.decode(response.body);
      ...
      var map = Map<String, dynamic>.from(jsonData);
      DoPandingListData = InwardListingResponse.fromJson(map);
