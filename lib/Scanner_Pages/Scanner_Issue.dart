...
class Scanner_Issue extends StatefulWidget {
  var menuName;
Scanner_Issue({Key? key,this.menuName}) : super(key: key);

  @override
  State<Scanner_Issue> createState() => _Scanner_IssueState();

}

class _Scanner_IssueState extends State<Scanner_Issue> {
  Barcode? result;
  late Tools tools;
    ...
    controller.scannedDataStream.listen((scanData) {
      ...
      setState(() {
        if(barcodeResponse!=""){
          ...
          setState(() {
              if (barcodeResponse.startsWith('M')) {
                setState(() {
                  GetURN_FORP(barcodeResponse);
                });

              } else if (barcodeResponse.startsWith('P')) {
                setState(() {
                  GetURN_FORP(barcodeResponse);
                });
  ...
  Future<void> GetURN_FORP(BarcodeRes) async {
    ...
    final response = await http.post(
      Uri.parse("${clientUrl}TechFlow/MaterialIssueSelection"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}TechFlow/MaterialIssueSelection $BODYDATA");
