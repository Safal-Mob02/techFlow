...
class ScannerForReceipt extends StatefulWidget {
  var menuName;
  ScannerForReceipt({Key? key,this.menuName}) : super(key: key);

  @override
  State<ScannerForReceipt> createState() => _ScannerForReceiptState();

}

class _ScannerForReceiptState extends State<ScannerForReceipt> {
  Barcode? result;
  late Tools tools;
    ...
    controller.scannedDataStream.listen((scanData) {
      ...
      setState(() {
        if(barcodeResponse!=""){
          ...
          setState(() {
            if(widget.menuName!="ProductionQC"){
              Navigator.push(context, MaterialPageRoute(builder: (context) => Material_Receipt_ItemList(Urn: widget.menuName, status: '',barcoderes:barcodeResponse)));
            }else{
              GetURN_FORP(barcodeResponse);
            }
  ...
  Future<void> GetURN_FORP(BarcodeRes) async {
    ...
    final response = await http.post(
      Uri.parse("${clientUrl}MaterialReceipt/ProductionOrderPendingEntryToEntryList"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}MaterialReceipt/ProductionOrderPendingEntryToEntryList $BODYDATA");
    ...
    if (response.statusCode == 200) {
      ...
      if (DoPandingListData.settings.success == "1") {
        ...
        setState(() {
          ...
          controller!.stopCamera();
          if(Urn.isNotEmpty){
            Navigator.push(context, MaterialPageRoute(builder: (context) => Material_Receipt_ItemList(Urn: Urn, status: '',)));
          }
        });
      } else {
        setState(() {
  ...
  Future<void> GetURN_FORS(BarcodeRes) async {
    setState(() {
      isLoading = true;
    });
    var BODYDATA = {
    ...
    final response = await http.post(
      Uri.parse("${clientUrl}TechFlow/MaterialRequisitionSelection"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}TechFlow/MaterialRequisitionSelection $BODYDATA");
    log("Response Status Code: ${response.statusCode}");
    log("Response Body: ${response.body}");
    if (response.statusCode == 200) {
      var jsonData = json.decode(response.body);
      ...
      if (DoPandingListData.settings.success == "1") {
        ...
        setState(() {
          ...
          controller!.stopCamera();
          if(Urn.isNotEmpty){
            Navigator.push(context, MaterialPageRoute(builder: (context) => ItemsPage(Urn: Urn,)));
          }
        });
      } else {
        if (!mounted) return;
