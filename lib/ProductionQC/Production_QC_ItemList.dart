...
import 'ProductionQC_EntryList.dart';
import 'Production_QC_ItemDetailsForm.dart';


class Production_QCItemList extends StatefulWidget {
  final TabController tabController;
  var urn,status;
  Production_QCItemList(
      {Key? key,
        required this.tabController, required this.urn,this.status
      })
      : super(key: key);
  @override
  State<Production_QCItemList> createState() => _Production_QCItemListState();
}

class _Production_QCItemListState extends State<Production_QCItemList> {
  var urCode;
    ...
    return WillPopScope(
      onWillPop: () async {
        return await Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) =>  ProductionQC_EntryList()));
      },
      child: Scaffold(
                                            ...
                                            Padding(
                                                ...
                                                child:Row(
                                                  children: [
                                                    ...
                                                    Flexible(
                                                      child: Text(
                                                        "${DoPandingListData.data[index].index}",
                                                        overflow:
                                                        TextOverflow
                                                            .visible,
                                                        style: TextStyle(
  ...
  Future<void> fetchdata(SearchText) async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        isLoading = false;
        DoPandingListData = PurchaseQcItemListResponse.fromJson(map);
      });
  ...
  Future<void> GenerateQCSummerySrNo() async {
    setState(() {
      isLoading = true;
    });
    ...
    final response = await http.post(
      Uri.parse("${clientUrl}/QC/GenerateQCSummerySrNo"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}/QC/GenerateQCSummerySrNo $BODYDATA");
    log("Response Status Code: ${response.statusCode}");
    log("Response Body: ${response.body}");
    if (response.statusCode == 200) {
      var jsonData = json.decode(response.body);
      log(jsonData.toString());
      var map = Map<String, dynamic>.from(jsonData);
      log("GenerateQCSummerySrNo body==>$map");
      setState(() {
        isLoading = false;
        //DoPandingListData = PurchaseQcItemListResponse.fromJson(map);
      });
      if (map['settings']['success'] == "1") {
        var srno =map['Data'][0]['SrNo'].toString();

        var customDatum = Datum(
          index: DoPandingListData.data.length + 1, // Next index
          status: "Accept",
          qty: 0.0,
          reason: "",
          SR_No: srno,
        ...
        // Add the customDatum to DoPandingListData.data
        setState(() {
          DoPandingListData.data.add(customDatum);
        });
        var index =DoPandingListData.data.length-1;
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) =>
                    Production_QC_ItemDetailsForm(
                      FormData:DoPandingListData.data[
                      index],
                      Urn: widget.urn,
                      // CategoryCode:
                      // CategoryCode,
