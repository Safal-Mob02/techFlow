...
import 'Producion_QC_Form_TAB_Page.dart';



class ProductionQC_EntryList extends StatefulWidget {
  const ProductionQC_EntryList({super.key});

  @override
  State<ProductionQC_EntryList> createState() => _ProductionQC_EntryListState();
}

class _ProductionQC_EntryListState extends State<ProductionQC_EntryList> {
  var urCode;
                            ...
                            return Dismissible(
                              confirmDismiss: (direction) async {
                                if (direction ==
                                  ...
                                  GetURN_FOR_ProductionQC("${DoPandingListData.data[index].urnNo}-${DoPandingListData.data[index].itemSrNo}");
          ...
          onPressed: () {
            setState(() {
              Navigator.push(context,
                  MaterialPageRoute(builder: (context) => ScannerForPurchaseQC(menuName: "ProductionQC",)));
              //  onNextPageChangeTapped();
            });
          },
          child: Container(
  ...
  Future<void> fetchdataForPending(SearchText) async {
    ...
    final response = await http.post(
      Uri.parse("${clientUrl}QC/QCProductionPendingData"),
      body: BODYDATA,
    );
    log("Api Name: ${clientUrl}QC/QCProductionPendingData $BODYDATA");
    log("Response Status Code: ${response.statusCode}");
    log("Response Body: ${response.body}");
    if (response.statusCode == 200) {
      var jsonData = json.decode(response.body);
      log(jsonData.toString());
      var map = Map<String, dynamic>.from(jsonData);
      DoPandingListData = ProductionQcPendingListResponse.fromJson(map);
      if (DoPandingListData.settings.success == "1") {
        if (!mounted) return;
        setState(() {
          isLoading = false;
